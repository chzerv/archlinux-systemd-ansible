# Archlinux Container Image for Ansible Testing

This Dockerfile builds an Archlinux based container, capable to use `systemd`, mainly for Ansible role testing.
...
2. Clone the repository: 
   ...
   ```
3. `cd` into the directory and run `docker build -t archlinux-systemd-ansible .`
...
3. Run a container:
   ...
   ```shell
   docker run -d --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro archlinux-systemd-ansible:latest
   ```
